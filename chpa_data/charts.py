...
def echarts_stackbar(df,  # 传入数据df，应该是一个行索引为date的时间序列面板数据
    ...
    if df.empty is False:
        ...
        stackbar.set_global_opts(
            ...
            tooltip_opts=opts.TooltipOpts(trigger='axis',# 触发类型。'axis': 坐标轴触发，主要在柱状图，折线图
                                                         # 等会使用类目轴的图表中使用。
                                          axis_pointer_type='cross',# 指示器类型。'cross'：十字准星指示器。其实是种简写，
                                                                    # 表示启用两个正交的轴的 axisPointer。
                                          ),
            xaxis_opts=opts.AxisOpts(type_='category',# 坐标轴类型。可选：'category': 类目轴，适用于离散的类目数据，
                                                      # 为该类型时必须通过 data 设置类目数据。
                                     boundary_gap=True,#类目轴中 boundaryGap默认为 true，这时候刻度只是作为分隔线,
